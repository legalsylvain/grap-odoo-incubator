* Go to "Settings > Technical > Parameters > System parameters" and
  ...
  - ``database_synchronization.host`` : host of the target Odoo instance, without
    ``http://``. set only ``localhost`` or the ``myerp.mywebsite.tld``.
  - ``database_synchronization.port``: port the target Odoo instance. Typically
    ``8069`` for dev instance, or ``443`` for test or production instance.
  - ``database_synchronization.database``: name of the database.
  - ``database_synchronization.login`` and ``database_synchronization.password``:
